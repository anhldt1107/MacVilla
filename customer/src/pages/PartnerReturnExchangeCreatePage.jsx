...
import { storeB2bCreateReturnExchangeRequest } from '../api/store/storeB2bReturnExchangeApi'
...
export function PartnerReturnExchangeCreatePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitBusy(true)
    try {
      const res = await storeB2bCreateReturnExchangeRequest(accessToken, body)
      const o = res && typeof res === 'object' ? /** @type {Record<string, unknown>} */ (res) : null
      const ticket = o?.ticketNumber != null ? String(o.ticketNumber) : ''
      if (ticket) {
        navigate(`/partner/after-sales/returns/${encodeURIComponent(ticket)}`, { replace: true })
      } else if (o?.id != null) {
        navigate('/partner/after-sales/returns', { replace: true })
      } else {
        navigate('/partner/after-sales/returns', { replace: true })
      }
    } catch (err) {
  ...
  return (
    <>
      <PartnerPaymentsPageHeader title="Tạo yêu cầu đổi / trả" />
