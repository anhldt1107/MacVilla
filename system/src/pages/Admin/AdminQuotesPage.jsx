...
import { ChevronDown, ChevronLeft, ChevronRight, FileSpreadsheet, Loader2, Plus, RefreshCw, Search } from "lucide-react";
import { SalerQuotationsNavTabs } from "@/components/Saler/SalerQuotationsNavTabs";
...
export function AdminQuotesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const view =
        paths.shell === "saler"
          ? salerQuotesMode === "queue"
            ? "queue"
            : salerQuotesMode === "mine"
              ? "mine"
              : "all"
          : undefined;
  ...
  }, [accessToken, isAuthenticated, page, pageSize, debouncedSearch, status, paths.shell, salerQuotesMode]);
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <CardTitle className="text-lg font-semibold tracking-tight">
                {loading ? "Danh sách báo giá (B2B)" : `Danh sách báo giá (B2B) · ${totalCount.toLocaleString("vi-VN")} báo giá`}
              </CardTitle>
            </div>
          </div>
                ...
                {!loading && items.length === 0 ? (
                  <tr>
                    <td colSpan={10} className="px-6 py-16 text-center">
                      ...
                      {paths.shell === "saler" && canCreateQuote ? (
                        <p className="mt-4">
                          <Link
                            to={`${paths.quotesList}/create`}
