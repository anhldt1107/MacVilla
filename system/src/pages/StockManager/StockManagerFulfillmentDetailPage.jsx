...
import { FulfillmentPickListCard, FulfillmentShippingCard } from "@/components/warehouse/FulfillmentPickListCard";
import { getFulfillmentOrderLines, getFulfillmentShippingAddress } from "@/lib/fulfillmentOrderUtils";
...
/**
 * GET chi tiết phiếu —  (chi tiết + đơn).
 * @param {object} [props]
 * @param {string} [props.dashboardHomeTo] mặc định `/stock-manager`
 * @param {string} [props.fulfillmentsListPath] mặc định `/stock-manager/fulfillments`
 * @param {string} [props.ordersPathPrefix] mặc định `/stock-manager/orders`
 * @param {string} [props.inventoryTxPath] mặc định `/stock-manager/inventory/transactions`
 * @param {boolean} [props.hideAssignWorker] ẩn gán worker (worker shell)
 * @param {boolean} [props.hideOrderDetailLink] ẩn link chi tiết đơn (worker không có route đơn)
 * @param {boolean} [props.workerMode] dialog cập nhật trạng thái chỉ bước kế tiếp, không hủy
 * @param {boolean} [props.hideInventoryTxLink] ẩn link giao dịch kho (worker shell)
 */
export function StockManagerFulfillmentDetailPage({
  dashboardHomeTo = "/stock-manager",
  fulfillmentsListPath = "/stock-manager/fulfillments",
  ordersPathPrefix = "/stock-manager/orders",
  inventoryTxPath = "/stock-manager/inventory/transactions",
  hideAssignWorker = false,
  hideOrderDetailLink = false,
  hideInventoryTxLink = false,
  workerMode = false,
} = {}) {
  ...
  const ordersPath =
    ordersPathPrefix != null && String(ordersPathPrefix).trim() !== ""
      ? String(ordersPathPrefix).replace(/\/$/, "")
      : null;
  ...
  const orderLines = getFulfillmentOrderLines(orderObj);
  const shippingAddress = getFulfillmentShippingAddress(orderObj);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 pb-10">
      ...
      <FulfillmentStatusUpdateDialog
        ...
        allowedTransitions={detail?.allowedFulfillmentTransitions}
      ...
      {!loading && !error && detail ? (
        <>
          <header className="border-b border-slate-200/90 pb-6 dark:border-slate-800">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {!hideAssignWorker && listRowForDialogs && isFulfillmentWorkerUnassigned(listRowForDialogs) ? (
          ...
          <FulfillmentPickListCard
            lines={orderLines}
            workerMode={workerMode}
            title={workerMode ? "Hàng cần lấy" : "Hàng cần lấy & gói"}
          />
            ...
            <Card className="border-slate-200/80 shadow-sm dark:border-slate-800">
              ...
              <CardContent className="space-y-3 text-sm">
                <div className="flex justify-between gap-4">
                  <span className="text-slate-500 dark:text-slate-400">Hình thức giao</span>
                  <span className="font-medium text-slate-900 dark:text-slate-100">{labelFulfillmentTicketType(ticketType)}</span>
                </div>
                <div className="flex justify-between gap-4">
              ...
              <CardContent className="space-y-3 text-sm">
                {orderObj ? (
                  <>
                    ...
                    {orderId != null && !hideOrderDetailLink && ordersPathPrefix ? (
                      <Button variant="outline" size="sm" className="mt-2 gap-1" asChild>
                        <Link to={`${ordersPathPrefix}/${encodeURIComponent(String(orderId))}`}>
                          Mở chi tiết đơn
                  ...
                  <p className="text-slate-500">Chưa có thông tin đơn hàng liên quan.</p>
                )}
              </CardContent>
            </Card>
          </div>

          <FulfillmentShippingCard address={shippingAddress} />
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {!hideInventoryTxLink && orderId != null && String(status).toLowerCase() === "shipped" ? (
              <Link
                to={inventoryTxPath}
