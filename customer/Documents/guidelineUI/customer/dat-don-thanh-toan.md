...
| `paymentMethod`     | string | Optional / **Required** — VD `COD`, `PayOS` |
...
Checkout và sổ địa chỉ dùng `AddressAutocompleteInput` → `GET /api/store/geo/autocomplete?input=...` (BE proxy Goong V2, **AllowAnonymous**).

- Dev: cấu hình `GOONG_API_KEY` trong `BE_HDGVH/.env` (xem `BE_HDGVH/.env.example`). Macvilla không cần biến env riêng.
- Khi chưa có key: form vẫn nhập tay; API trả `503 GOONG_NOT_CONFIGURED`, FE degrade im lặng.
- Giá trị lưu DB vẫn là `addressLine` (chuỗi `description` từ Goong).
