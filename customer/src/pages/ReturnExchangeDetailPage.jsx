...
import {
  storeMeCancelReturnExchangeRequest,
  storeMeFetchReturnExchangeByTicket,
} from '../api/store/storeMeReturnExchangeApi'
...
export function ReturnExchangeDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const d = await storeMeFetchReturnExchangeByTicket(accessToken, ticketNumber)
  ...
  const handleCancel = async () => {
    ...
    setError('')
    try {
      const updated = await storeMeCancelReturnExchangeRequest(accessToken, ticketNumber)
    ...
    <AccountAccountShell
      hero={
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white mt-4">Chi tiết phiếu đổi / trả</h1>
        </div>
      }
    >
