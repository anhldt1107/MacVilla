...
import { ChevronDown, ChevronLeft, ChevronRight, Loader2, PenSquare, Plus, RefreshCw, Search } from "lucide-react";
import { OrderStatusUpdateDialog } from "@/components/Admin/orders/OrderStatusUpdateDialog";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { cn } from "@/lib/utils";
import { staffOrderDetailHref, staffOrderListRowCode, staffOrderListRowId } from "@/lib/staffOrderRoutes";
...
function paymentMethodPillClass(method) {
  const key = String(method ?? "")
    .replace(/\s+/g, "")
    .toLowerCase();
  if (key === "cod") {
    return "bg-orange-50 text-orange-900 ring-1 ring-orange-200/80 dark:bg-orange-950/40 dark:text-orange-100 dark:ring-orange-800/60";
  }
  if (key === "payos" || key.includes("payos")) {
    return "bg-violet-50 text-violet-900 ring-1 ring-violet-200/80 dark:bg-violet-950/40 dark:text-violet-100 dark:ring-violet-800/60";
  }
  return "bg-slate-100 text-slate-700 ring-1 ring-slate-200/80 dark:bg-slate-800 dark:text-slate-200 dark:ring-slate-700";
}

/** @param {import("@/services/admin/adminOrdersApi").AdminOrderListItem | Record<string, unknown>} row */
function orderListPaymentMethod(row) {
  const r = /** @type {Record<string, unknown>} */ (row);
  const v = r.paymentMethod ?? r.PaymentMethod;
        ...
        <CardHeader className="flex flex-col gap-4 border-b border-slate-100 bg-white pb-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800 dark:bg-slate-950">
          <div>
            <CardTitle className="text-lg font-semibold tracking-tight">
              {loading ? "Danh sách đơn hàng" : `Danh sách đơn hàng · ${totalCount.toLocaleString("vi-VN")} đơn`}
            </CardTitle>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            {isSalesShell ? (
              <Button type="button" size="sm" className="gap-1.5 bg-violet-600 hover:bg-violet-700 dark:bg-violet-600 dark:hover:bg-violet-500" asChild>
                <Link to={`${paths.ordersList}/create`}>
            ...
            <table className="w-full min-w-[1080px] border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/95 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-900/80 dark:text-slate-400">
                  ...
                  <th className="px-5 py-3.5">PTTT</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                {items.map((row, idx) => {
                  const orderId = staffOrderListRowId(row);
                  const detailPath = staffOrderDetailHref(paths, {
                    orderId,
                    orderCode: staffOrderListRowCode(row),
                  });
                  return (
                  <tr
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 align-middle">
                      {(() => {
                        const pm = orderListPaymentMethod(row);
                        return (
                          <span
                            className={cn(
                              "inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold",
                              paymentMethodPillClass(pm)
                            )}
                            title={pm || undefined}
                          >
                            {labelOrderPaymentMethod(pm)}
                          </span>
                        );
                      })()}
                    </td>
