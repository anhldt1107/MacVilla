...
/**
 * Stock Manager / Worker — danh sách phiếu xuất ().
 * @param {object} [props]
 * @param {string} [props.initialAssignedWorkerId] — khi có, cố định bộ lọc Worker
 * @param {string} [props.fulfillmentsBasePath] — prefix danh sách + chi tiết (mặc định `/stock-manager/fulfillments`)
 * @param {string | null} [props.ordersPathPrefix] — link «Xem đơn» (mặc định `/stock-manager/orders`)
 * @param {boolean} [props.hideOrderLinks] ẩn link đơn (worker shell)
 * @param {boolean} [props.workerMode] dialog status chỉ bước kế (worker)
 * @param {boolean} [props.hideAssignWorker] ẩn gán worker (worker shell)
 * @param {string} [props.fullFulfillmentsPath] — khi đang lọc theo worker: «Xem tất cả phiếu» (mặc định = fulfillmentsBasePath)
 */
export function StockManagerFulfillmentsPage({
  initialAssignedWorkerId,
  fulfillmentsBasePath = "/stock-manager/fulfillments",
  ordersPathPrefix = "/stock-manager/orders",
  hideOrderLinks = false,
  workerMode = false,
  hideAssignWorker = false,
  fullFulfillmentsPath: fullFulfillmentsPathProp,
} = {}) {
  const fulfillmentsHub = String(fulfillmentsBasePath).replace(/\/$/, "");
  const ordersBase =
    ordersPathPrefix != null && String(ordersPathPrefix).trim() !== ""
      ? String(ordersPathPrefix).replace(/\/$/, "")
      : null;
  const showAllFulfillmentsPath =
    fullFulfillmentsPathProp != null && String(fullFulfillmentsPathProp).trim() !== ""
      ? String(fullFulfillmentsPathProp).replace(/\/$/, "")
      : fulfillmentsHub;
  const showViewAllFulfillmentsLink = showAllFulfillmentsPath !== fulfillmentsHub;
  ...
  /** `queue` = hàng đợi kho (`status=Pending`), `all` = dùng bộ lọc trạng thái bên dưới — . */
  const [listMode, setListMode] = useState(/** @type {"all" | "queue"} */ (() =>
    searchParams.get("queue") === "pending" ? "queue" : "all"
  ));
                ...
                {lockedWorkerId ? (
                  <p className="mt-2 text-xs sm:text-sm">
                    <span className="rounded-md bg-amber-50 px-2 py-1 font-medium text-amber-900 ring-1 ring-amber-200/80 dark:bg-amber-950/30 dark:text-amber-100 dark:ring-amber-800/50">
                      Phiếu của: {user?.fullName?.trim() || lockedWorkerId}
                    </span>
                    {showViewAllFulfillmentsLink ? (
                      <Link
                        className="ml-2 font-medium text-teal-700 underline-offset-2 hover:underline dark:text-teal-400"
                        to={showAllFulfillmentsPath}
                      >
                        Xem tất cả phiếu
                      </Link>
                    ) : null}
        ...
        <CardHeader className="flex flex-col gap-4 border-b border-slate-100 bg-white pb-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800 dark:bg-slate-950">
          <div>
            <CardTitle className="text-lg font-semibold tracking-tight">
              {loading ? "Danh sách phiếu công việc" : `Danh sách phiếu công việc · ${totalCount.toLocaleString("vi-VN")} phiếu`}
            </CardTitle>
          </div>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                {items.map((row, idx) => (
                  <tr
                    ...
                    onClick={() => navigate(`${fulfillmentsHub}/${row.id}`)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        navigate(`${fulfillmentsHub}/${row.id}`);
                      }
                    }}
                  >
                    ...
                    <td className="px-4 py-3.5 align-middle text-sm text-slate-800 dark:text-slate-200">
                      {labelFulfillmentTicketType(row.ticketType)}
                    </td>
                    <td className="whitespace-nowrap px-4 py-3.5 align-middle">
                      <button
                        type="button"
                      ...
                      {isFulfillmentWorkerUnassigned(row) ? (
                        <div className="flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
                          ...
                          >
                            ...
                            <span className="text-xs text-slate-500">Chưa gán Worker</span>
                          )}
                          {user?.id ? (
                            <Button
                              type="button"
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5 pr-6 text-right align-middle">
                      {!hideOrderLinks && ordersBase ? (
                        <Button variant="outline" size="sm" className="h-8 gap-1 px-2.5" asChild>
                          <Link to={`${ordersBase}/${row.orderId}`} onClick={(e) => e.stopPropagation()}>
                            Xem đơn
