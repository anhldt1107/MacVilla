...
Sales **theo dõi đơn** do mình tạo (từ convert báo giá hoặc tạo hộ khách), **hỗ trợ khách** về trạng thái, **hủy** khi được phép. **Xem** phiếu xuất: `GET /api/admin/fulfillments?orderId=...` (StaffAuthenticated). Thao tác kho (tạo phiếu, đổi trạng thái, gán worker) thuộc **WarehouseStaff** ([../admin/fulfillment.md](../admin/fulfillment.md)).
