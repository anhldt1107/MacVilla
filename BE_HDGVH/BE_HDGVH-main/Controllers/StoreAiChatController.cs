...
public class StoreAiChatController(IAiAssistantService assistant, BeContext db) : ControllerBase
{
    [HttpPost("chat")]
    [SwaggerOperation(Summary = "Trợ lý Gemini cho khách lẻ B2C (tra đơn của chính khách + tìm sản phẩm)")]
    ...
    private async Task<AiCallerContext> BuildCallerAsync(CancellationToken cancellationToken)
    {
        ...
        var customer = await db.Customers.AsNoTracking()
            ...
            .Select(c => new { c.CustomerType, Name = c.FullName })
            .FirstOrDefaultAsync(cancellationToken)
            ?? throw new UnauthorizedAccessException("Không tìm thấy khách hàng.");

        if (!string.Equals(customer.CustomerType, CustomerTypes.B2C, StringComparison.OrdinalIgnoreCase))
            throw new UnauthorizedAccessException("Endpoint này dành cho khách lẻ B2C; khách B2B vui lòng dùng /api/store/b2b/ai/chat.");

        return AiCallerContext.ForCustomer(customerId, isB2B: false, customer.Name);
    }
}
